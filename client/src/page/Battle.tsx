...
import { attack, attackSound, defense, defenseSound, xho } from '../assets';
import { ActionButton, Alert, Card, GameInfo } from '../components';
import PlayerInfo from '../components/PlayerInfo';
import { useGlobalContext } from '../context';
import styles from '../styles';
import { player01 as player1Icon, player02 as player2Icon } from '../assets';
...
const Battle = () => {
    ...
    useEffect(() => {
        const getPlayerInfo = async () => {
            try {
                ...
                const p1TokenData = await contract.getPlayerToken(
                    player01Address
                );
    ...
    const makeAmove = async (choice) => {
        ...
        try {
            ...
            setShowAlert({
                status: true,
                type: 'info',
                message: `Initiating ${choice ? 'attack' : 'defense'}`,
            });
        } catch (error) {
            setErrorMessage(error);
        }
    ...
    return (
        ...
        >
            ...
            <PlayerInfo player={player1} playerIcon={player1Icon} />
            <GameInfo />
        </div>
    );
};
